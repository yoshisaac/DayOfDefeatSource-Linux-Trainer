...
void aimbot(pid_t gamePid, Display* aimDisplay) {
  for (;;) {
    for (int i = 0; i < 32; ++i) {
      ...
      WorldToScreen(gamePid, player.boneMatrix[15], enemy_screen);
	...
	if (AIMBOT::aimIndex == i) {
	  ...
	  float deltaLocation[3] = { float(p_local.absLocation[0] - player.boneMatrix[15][0]),
				     float(p_local.absLocation[1] - player.boneMatrix[15][1]),
				     float((p_local.absLocation[2] + p_local.height - 15 ) - player.boneMatrix[15][2]) };
